...
function orderSummary(){
    for(let i = 0; i < tellimus.rows.length; i++){
        summ = ((tellimus.rows[i].price.toFixed(2) * tellimus.rows[i].amount))
        console.log("Toode:" + tellimus.rows[i].name)
        console.log("Kogus:" + tellimus.rows[i].amount)
        console.log("Hind:" + tellimus.rows[i].price.toFixed(2) + "EUR")
        console.log("Summa:" + summ + "EUR")
    }
}

let kogusumma = 0
  for (let i = 0; i < tellimus.rows.length; i++) {
    kogusumma+= ((1 + tellimus.rows[i].vat) * (tellimus.rows[i].price * tellimus.rows[i].amount))
    }    
orderSummary()
console.log("Kogusumma käibemaksuga:" , kogusumma.toFixed(2) + "EUR")
